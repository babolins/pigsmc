...
5. As a researcher, I want to specify boundary conditions via a named enum (`Boundary.FREE`, `Boundary.PERIODIC_3D`, `Boundary.QUASI_2D`, `Boundary.QUASI_1D`) and orthorhombic box lengths, so that I can simulate free, fully periodic, slab, and tube geometries without implementing minimum-image logic myself.
...
22. As a researcher, I want `PathState` (passed to moves) to expose positions, orientations, N, M, τ', per-particle λ values, boundary information, a `slice_kind` array, and writable `buffer_positions` and `buffer_orientations` arrays of shape (M, 1, 3), so that my custom move has all the information it needs to construct a valid proposal and a pre-allocated destination to write proposed coordinates into without touching the main path arrays.
23. As a researcher, I want `MoveResult` to carry the list of changed (particle, slice) pairs written into the move buffer and a `log_ratio_contrib` scalar for any kinematic contribution the move owns (e.g. the Lévy bridge factor in bisection), so that the engine can read the buffer to compute potential and wavefunction differences and copy it into the main path arrays only on acceptance — leaving the main path arrays unchanged on rejection.
...
- **Move proposal**: `propose()` writes proposed coordinates into `PathState.buffer_positions` and `PathState.buffer_orientations` (pre-allocated (M, 1, 3) arrays, reused across all move attempts), then returns `MoveResult` with `changed: list[tuple[int, int]]` (particle, slice pairs written to the buffer) and `log_ratio_contrib: float`. The engine reads the buffer to compute potential and wavefunction differences; on acceptance it copies the buffer into the main path arrays; on rejection the main path arrays are left unchanged.
